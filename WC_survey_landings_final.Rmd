...
# Three bio geographic regions were defined using published literature suggesting potentially significant faunal changes within these regions (Gabriel 1982, Williams & Ralston 2002, Francis et al. 2009) The biogeographic regions are: south of Cape Mendocino, Cape Mendocino to Cape Blanco, north of Cape Blanco

sppDeepWC <- capturas %>%
  filter(depth_m < 600) %>%   #REMEMBER TO DELETE THIS COMMAND
...
```{r cache=TRUE}
fishbaseInfo1 <- species(sppDeepWC$cientifico)
fishbaseInfo2 <- left_join(sppDeepWC, fishbaseInfo1, by=c("cientifico"="Species")) %>%
  filter(SpecCode != "NA")

sppDeepWC2 <- inner_join(sppDeepWC, fishbaseInfo2, by = "cientifico") # getting traits from intersection with surveys traits

temp <- ecology(sppDeepWC2$cientifico, fields=c("SpecCode", "DietTroph")) %>%
    rename(TL=DietTroph)
fishbaseInfo <- left_join(fishbaseInfo2, temp, by=c("SpecCode"="SpecCode"))
...
cData2 <- cData[-58,]
cData2 <- read.csv('Data/Derived/WC_survey_traits_0_600_35_40_final.csv') %>%
...
row.names(cData2) <- cData2$cientifico
...
```{r}
cData3 <- cData2 %>%  #read.csv('Data/Derived/WCsurveytraits2.csv', stringsAsFactors = F)
  select(cientifico)

yearCatch1 <- capturas %>%
  filter(depth_m < 600) %>%
  filter(latitude_dd >35) %>%
  filter(latitude_dd <40) %>%
  filter(!year %in% c("1977","1980","1983","1986", "1989","1992", "1995", "1998", "1999", "2000", "2001", "2002")) %>% #selecting years for the current trawling survey sampling 2003 - 2013(cite)
...
# cData4 <- read.csv('Data/Derived/WC_survey_traits_0_600_35_40_final.csv')
# cData5 <- read.csv('Data/Derived/WC_survey_traits_0_600_35_40.csv') %>%
# rename(cientifico=X)
...
nameList <- cData2$cientifico
cData2 <- cData2[,-1]
row.names(cData2) <- nameList

testFDcommunity <- dbFD(cData2, corr = "cailliez", a=abundances)
datatable(data.frame(testFDcommunity[1:8]))
datatable(testFDcommunity$CWM)


write.csv(cData2, file="Data/Derived/WC_survey_traits_0_600_35_40.csv")
write.csv(abundances, file="Data/Derived/WC_survey_abundances_0_600_35_40.csv")
write.csv(testFDcommunity, file="Data/Derived/WC_survey_FD_0_600_35_40.csv")
```
...
Landingstraits <- fbSpTrawling %>%
  filter(DemersPelag %in%c('demersal', 'bathydemersal', 'reef-associated'))   #pelagic-neritic
...
# spfromsurvey <- cData2 %>%
#   select(cientifico) %>%
#   rename(spName = cientifico)
# 
# surveytraits <- inner_join(fbSpTrawling, spfromsurvey, by = "spName")

row.names(Landingstraits) <- Landingstraits$cientifico
Ltraits <- Landingstraits %>%
  select(-cientifico)
